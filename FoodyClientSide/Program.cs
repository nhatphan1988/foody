...
namespace FoodyClientSide
{
    class Program
    {
        ...
        static async Task TestValuesController()
        {
            string hostUriString = "http://localhost:42217";
            HttpResponseMessage response;
            using (var client = new HttpClient())
            {
                var tokenEndpoint = new Uri(new Uri(hostUriString), "api/Values");
                response = await client.GetAsync(tokenEndpoint.AbsoluteUri);
            }
